...
<div class="d-flex justify-content-between align-items-start mb-4">
    <div>
        <h1 class="h2 mb-1">{{ __('Edit User') }}</h1>
        <p class="text-muted mb-0">{{ __('Update user account information') }}</p>
    </div>
    <div class="d-flex gap-2" style="margin-top: 0.5rem;">
        <a href="{{ route('users.show', $user) }}" class="btn btn-outline-info">
            <i class="bi bi-eye me-2"></i>{{ __('View User') }}
        </a>
    ...
    <div class="col-lg-8">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">
                    <i class="bi bi-person-gear me-2"></i>{{ __('User Information') }}
                </h5>
            </div>
            <div class="card-body">
                <form action="{{ route('users.update', $user) }}" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <small class="text-muted">{{ __('Minimum 6 characters. Leave blank to keep current password.') }}</small>
                            </div>
                        </div>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                <label for="password_confirmation" class="form-label fw-medium">
                                    <i class="bi bi-lock-fill me-1"></i>{{ __('Confirm New Password') }}
                                </label>
                                <div class="input-group">
                                    <input type="password" 
                                           class="form-control" 
                                    ...
                                    @foreach($regions as $region)
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input region-checkbox" type="checkbox" 
                                                   ...
                                                   {{ in_array($region->id, $userRegionIds) ? 'checked' : '' }}>
                                    ...
                                    @foreach($channels as $channel)
                                        ...
                                        <div class="form-check">
                                            <input class="form-check-input channel-checkbox" type="checkbox" 
                                                   ...
                                                   {{ in_array($channel->id, $userChannelIds) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('classification') is-invalid @enderror" id="classification" name="classification">
                                    ...
                                    <option value="food" {{ old('classification', $user->classification) === 'food' ? 'selected' : '' }}>
                                        {{ __('Food') }}
                                    </option>
                                    <option value="non_food" {{ old('classification', $user->classification) === 'non_food' ? 'selected' : '' }}>
                                        {{ __('Non-Food') }}
                                    </option>
                                    <option value="both" {{ old('classification', $user->classification) === 'both' ? 'selected' : '' }}>
                                        {{ __('Both') }}
                                    </option>
                                </select>
                    ...
                    <div class="d-flex gap-2">
                        <button type="submit" class="btn btn-primary">
                            <i class="bi bi-check-circle me-2"></i>{{ __('Update User') }}
                        </button>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="mb-3">
                    ...
                    <div class="fw-medium">{{ $user->updated_at->format('M d, Y H:i') }}</div>
                </div>
                
                @if($user->id === auth()->id())
                    <div class="alert alert-warning">
                        <i class="bi bi-exclamation-triangle me-2"></i>
                        <small><strong>{{ __('Note:') }}</strong> {{ __('You are editing your own account.') }}</small>
                    </div>
                @endif
            </div>
        </div>
